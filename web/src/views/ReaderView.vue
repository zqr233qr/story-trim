...
const fetchChapterContent = async (index: number) => {
  ...
  const pID = selectedPromptID.value
  const pVer = selectedPrompt.value?.version || 'v1.0'

  isChapterLoading.value = !chap.content 
  try {
    // 重点修复：传递当前选中的 promptID
    const res = await api.getChapter(chap.id, pID, pVer) 
    if (res.data.code === 0) {
      const data = res.data.data
      bookStore.updateChapterContent(index, data.content)
...
const processChapter = async (index: number) => {
  ...
  if (!chap || (trimmedCache.value[pID] && trimmedCache.value[pID][index])) return
  ...
  await api.trimStream(
    ...
    () => {
      if (!trimmedCache.value[pID]) trimmedCache.value[pID] = {}
      trimmedCache.value[pID][index] = streamingContent.value
      streamingContent.value = ''
...
const exportNovel = () => {
  ...
  chapters.value.forEach((chap, idx) => {
    content += `### ${chap.title}\n\n`
    content += (trimmedCache.value[pID] && trimmedCache.value[pID][idx]) ? trimmedCache.value[pID][idx] : `[未处理原文]\n${chap.content}`
    content += `\n\n${'-'.repeat(20)}\n\n`
  })
...
onMounted(async () => {
  ...
  // 2. 恢复模式偏好
  if (bookStore.lastPromptID) {
    selectedPromptID.value = bookStore.lastPromptID
  }

  // 3. 恢复章节进度
  if (bookStore.lastReadChapterID) {
    const idx = chapters.value.findIndex(c => c.id === bookStore.lastReadChapterID)
    if (idx >= 0) {
...
watch([readingMode, viewMode, selectedPromptID], () => {
  ...
  // 重点修复：模板变化时重新拉取当前章内容（以获取对应模式的缓存）
...
<template>
  <div class="h-screen flex flex-col bg-[#F9F7F1] overflow-hidden select-none font-sans text-slate-900">
    ...
    <!-- 顶部导航 -->
    <header 
      class="h-14 bg-white/90 backdrop-blur-md border-b border-gray-200/50 flex items-center justify-between px-4 lg:px-6 shrink-0 fixed top-0 left-0 right-0 z-30 transition-transform duration-300"
    ...
    >
      <div class="flex items-center gap-2 lg:gap-4 flex-1 min-w-0">
        ...
        <button @click="router.push('/dashboard')" class="hidden lg:block p-2 hover:bg-black/5 rounded-full transition-colors"><ArrowLeft class="w-5 h-5 text-gray-600" /></button>
        <div class="min-w-0">
      ...
      <div class="flex items-center gap-1">
        <button @click="startTrim" :disabled="isTrimming || isCurrentChapterTrimmed" class="p-2 transition-colors" :class="isCurrentChapterTrimmed ? 'text-teal-500' : 'text-gray-400'">
          <Loader2 v-if="isTrimming" class="w-4 h-4 animate-spin" /><Scissors v-else class="w-4 h-4" />
        </button>
        <button @click="router.push('/dashboard')" class="bg-black text-white p-2 rounded-full hover:bg-gray-800 transition-colors ml-1"><X class="w-4 h-4" /></button>
      </div>
    </header>
    ...
    <div class="flex-1 overflow-hidden flex relative" @mousedown="handleScreenClick">
      <aside class="bg-white border-r border-gray-100 flex flex-col shrink-0 fixed inset-y-0 left-0 w-72 z-50 transition-transform duration-300 transform lg:static lg:translate-x-0" :class="showSidebar ? 'translate-x-0' : '-translate-x-full'">
        ...
        <div class="p-4 overflow-y-auto custom-scrollbar flex-1 space-y-1">
          <button v-for="(chap, idx) in chapters" :key="idx" @click.stop="selectChapter(idx)" class="w-full text-left px-3 py-3 lg:py-2 rounded-lg text-sm transition-colors flex justify-between items-center group" :class="selectedIndex === idx ? 'bg-teal-50 text-teal-700 font-bold' : 'text-gray-500 hover:bg-gray-50'"><span class="truncate pr-4 text-[13px] lg:text-sm">{{ chap.title }}</span><CheckCircle2 v-if="bookStore.trimmedIDs.includes(chap.id)" class="w-3.5 h-3.5 text-teal-500 shrink-0" /></button>
        </div>
      </aside>

      <main class="flex-1 flex overflow-hidden relative bg-[#F9F7F1]">
               ...
               <article class="prose prose-slate prose-lg font-serif leading-loose text-gray-800 whitespace-pre-wrap">
                 ...
                 <div v-else class="flex flex-col items-center justify-center h-64 border-2 border-dashed border-gray-200 rounded-2xl text-gray-400">
                    ...
                    <div class="flex gap-3 mt-4">
                      <button @click.stop="startTrim" class="px-6 py-2 bg-teal-600 text-white rounded-full text-xs font-bold shadow-lg shadow-teal-600/20 active:scale-95 transition-all">立即精简本章</button>
                      <button @click.stop="toggleBatchProcess" class="px-6 py-2 bg-slate-800 text-white rounded-full text-xs font-bold shadow-lg shadow-slate-800/20 active:scale-95 transition-all flex items-center gap-2">
                        <Play class="w-3 h-3 text-teal-400" />
                        精简全书
                      </button>
                    </div>
                 </div>
        ...
        <div v-else ref="trimmedContainer" class="flex-1 overflow-y-auto scroll-smooth pt-14 px-6 lg:px-12 py-10 lg:py-14">
          <div v-if="viewMode === 'split'" class="flex flex-row min-h-full divide-x divide-gray-200/50">
             ...
             <section class="flex-1 p-4 lg:p-8">
                <article v-if="currentDisplayContent" class="prose prose-slate prose-lg font-serif leading-loose text-gray-800 whitespace-pre-wrap">{{ currentDisplayContent }}</article>
                <div v-else class="flex flex-col items-center justify-center h-full text-gray-400">
                   <Scissors class="w-8 h-8 mb-2 opacity-50" /><button @click="startTrim" class="text-xs text-teal-600 font-bold hover:underline">点击精简本章</button>
                </div>
             </section>
          </div>
          <div v-else class="max-w-2xl mx-auto p-8 lg:p-12 min-h-full">
             <article v-if="viewMode === 'original' || currentDisplayContent" class="prose prose-slate prose-lg font-serif leading-loose text-gray-800 whitespace-pre-wrap">{{ viewMode === 'original' ? selectedChapter?.content : currentDisplayContent }}</article>
             <div v-else class="flex flex-col items-center justify-center h-96 border-2 border-dashed border-gray-200 rounded-3xl text-gray-400">
                <Scissors class="w-12 h-12 mb-4 opacity-20" />
                <p class="text-sm">尚未精简，点击上方工具栏按钮开始</p>
                <div class="flex gap-4 mt-6">
                  <button @click="startTrim" class="px-8 py-2.5 bg-teal-600 text-white rounded-full text-sm font-bold shadow-xl shadow-teal-600/30 active:scale-95 transition-all">立即精简</button>
                  <button @click="toggleBatchProcess" class="px-8 py-2.5 bg-slate-800 text-white rounded-full text-sm font-bold shadow-xl shadow-slate-800/20 active:scale-95 transition-all flex items-center gap-2">
                    <Play class="w-4 h-4 text-teal-400" />
                    精简全书
                  </button>
                </div>
             </div>
