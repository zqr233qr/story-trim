...
export class MiniDataProvider implements IDataProvider {
  ...
  async getTrimmedStatus(book: Book, chapters: Chapter[]): Promise<TrimmedStatusMap> {
    try {
      ...
      console.error('[MiniDataProvider] Failed to sync trimmed status:', e);
      return {};
    }
  }
  ...
  async getTrimmedContent(
    ...
    const cached = uni.getStorageSync(STORAGE_KEYS.TRIM(cacheKey, promptId));
    if (cached) {
      return {
        content: cached,
        cached: true,
    ...
    try {
      const remoteResults = await ContentAPI.getBatchTrimmedById([chapter.cloud_id], promptId);
      if (remoteResults.data.length > 0 && remoteResults.data[0].trimmed_content) {
        const content = remoteResults.data[0].trimmed_content;
        
        uni.setStorage({
          key: STORAGE_KEYS.TRIM(cacheKey, promptId),
          data: content
        });
      ...
      console.error('[MiniDataProvider] Failed to fetch trimmed content:', e);
    }
  ...
  async getBatchTrimmedContents(
    ...
    if (missingChapters.length > 0) {
      ...
      try {
        const remoteResults = await ContentAPI.getBatchTrimmedById(ids, promptId);
        
        for (const remote of remoteResults.data) {
          const chapter = chapters.find(ch => ch.cloud_id === remote.chapter_id);
          if (chapter) {
            const cacheKey = (chapter.chapter_md5 || chapter.md5 || '');
            uni.setStorage({
              key: STORAGE_KEYS.TRIM(cacheKey, promptId),
              data: remote.trimmed_content
            });
          }
        }
        
        const remoteMap = new Map(remoteResults.data.map(r => [r.chapter_id, r.trimmed_content]));
        for (const chapter of missingChapters) {
          const idx = chapters.indexOf(chapter);
          if (idx >= 0) {
            results[idx] = {
              content: remoteMap.get(chapter.cloud_id || 0) || '',
              cached: false,
              source: 'network' as const
            };
          }
        ...
        console.error('[MiniDataProvider] Failed to fetch remote trimmed content:', e);
      }
    }
  ...
  async saveTrimmedContent(
    ...
    uni.setStorage({
      key: STORAGE_KEYS.TRIM(chapterMd5, promptId),
      data: content
    });
  }
